...
public class ViewHelper
{
	...
	public static boolean isValidPassword(String password) throws ParamInitException
	{
		...
		isNotEmptyPassword(password);
		// remove whitespaces
		password = password.trim();
		int len = password.length();
		if (len < MIN_PASSWORD_LENGTH || len > MAX_PASSWORD_LENGTH)
		{
			throw new ParamInitException("Invalid Password", "Password must have at least " + MIN_PASSWORD_LENGTH
		...
		if (qtyUppercase < MIN_UPPERCASE_CHARS)
			throw new ParamInitException("Invalid Password", "Password must contain at least " + MIN_UPPERCASE_CHARS
					+ " uppercase characters");
		if (qtyDigits < MIN_DIGITS)
			throw new ParamInitException("Invalid Password", "Password must contain at least " + MIN_DIGITS + " digits [0-9]");
		return true;
	}

	public static boolean isNotEmptyPassword(String password) throws ParamInitException
	{
		if (!isFieldAValidString(password))
		{
			throw new ParamInitException("Missing Parameter", "Password is missing or invalid");
		}
		return true;
	}
